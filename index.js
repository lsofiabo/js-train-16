...
function filterStudentsByGrade(students, grade) {
  ...
  students.filter( x => {
    if (x.grade === grade) {
      console.log(`${x.name} has ${grade} grade`);
    }
  });
...
function warnAboutMaxArrayLength(arr, maxLength) {
  if (!Array.isArray(arr) || isNaN(maxLength)) {
    console.error('First element must be an array, and the second must be a number!');
    return null;
  }
  if (arr.length > maxLength) {
    console.warn('Warning! Array length is above maximum!');
  } else {
    console.log('Array length is below maximum.');
  }
...
function compareArrays(arr1, arr2) {
  ...
  console.assert(JSON.stringify(arr1) === JSON.stringify(arr2), 'arr1 is not equal to arr2');
...
function validateUserInput(userInput) {
  ...
  if (userInput.password.length < 8) {
    console.warn('Warning: password length should be longer than 8 characters!');
  }
...
function calculateTotalPrice(products) {
  ...
  console.info(`Total price of products:`, total);
